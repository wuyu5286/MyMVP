...
import com.jumeng.shop.widgets.recycler.WrapRecyclerView;
...
public class DefaultItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public DefaultItemDecoration(RecyclerView recyclerView, Drawable dividerVertical, Drawable dividerHorizontal, int dividerDrawableSizeVertical, int dividerDrawableSizeHorizontal) {
    ...
    private void drawDividerDrawable(Canvas c, RecyclerView parent) {
        ...
        if (parent instanceof WrapRecyclerView) {
            ...
            footersCount = curWrapRecyclerView.getFooterViewsCount();
            itemViewCount = curWrapRecyclerView.getAdapter().getItemCount() - headersCount - footersCount;
        } else {
        ...
        for (int i = 0; i < parent.getChildCount(); i++) {
            ...
            // intercept filter
            if (isInterceptFilter(position, headersCount, footersCount, itemViewCount)) continue;

            if (mLayoutManagerType == WrapRecyclerView.LAYOUT_MANAGER_TYPE_LINEAR && (!isHeaderDividersEnabled || headersCount == 0) && position - headersCount == 0) {
                continue;
            }
            ...
            if (isHeaderViewPos(headersCount, position) || isFooterViewPos(headersCount, footersCount, itemViewCount, position) || isEmptyView) {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        // intercept filter
        if (isInterceptFilter(position, headersCount, footersCount, itemViewCount)) return;

        // set headView or footerView
        if (isHeaderViewPos(headersCount, position)) {
            // head
            if (mOrientation == OrientationHelper.HORIZONTAL) {
            ...
            return;
        } else if (isFooterViewPos(headersCount, footersCount, itemViewCount, position)) {
            // footer
            if (mOrientation == OrientationHelper.HORIZONTAL) {
                outRect.set(mVerticalDividerDrawableHeight, 0, 0, 0);
            } else {
    ...
    private boolean isHeaderViewPos(int headersCount, int position) {
        return isHeaderDividersEnabled && headersCount > 0 && position < headersCount;
    }

    private boolean isFooterViewPos(int headersCount, int footersCount, int itemViewCount, int position) {
        return isFooterDividersEnabled && footersCount > 0 && position >= itemViewCount + headersCount;
    }

    private boolean isInterceptFilter(int position, int headersCount, int footersCount, int itemViewCount) {
        ...
        if ((!isFooterDividersEnabled || footersCount == 0) && position >= itemViewCount + headersCount)
            return true;
    ...
    private boolean isEmptyView(WrapRecyclerView wrapRecyclerView, int position, int headersCount) {
        if (null != wrapRecyclerView && wrapRecyclerView.isKeepShowHeadOrFooter() && wrapRecyclerView.isShowEmptyView() && position == headersCount) {
            return true;
        }
